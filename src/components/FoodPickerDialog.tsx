  ...
  return (
    <>
      <Dialog open={open} onOpenChange={(v) => (v ? onOpenChange(true) : close())}>
        <DialogContent
          className={cn(
            "grid-cols-1 top-2 translate-y-0 w-[calc(100vw-1.5rem)] max-h-[92vh] gap-4 overflow-y-auto overflow-x-hidden p-4 text-right sm:top-1/2 sm:-translate-y-1/2 sm:w-full sm:max-h-[92vh] sm:p-6",
            selected ? "sm:max-w-xl" : "sm:max-w-lg",
          )}
        >
          <DialogHeader className="text-center sm:text-center">
            <DialogTitle className="text-center">
              {selected ? "בחירת כמות" : MEALS.find((m) => m.key === meal)?.label ?? "הוספת מזון"}
            </DialogTitle>
                  ...
                  <details className="rounded-2xl border border-border p-3">
                    ...
                    <div className="mt-3 space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <Button
                          className="w-full rounded-full text-xs bg-primary text-primary-foreground hover:bg-primary/90"
                          size="sm"
          ...
          {selected ? (
            <div className="flex min-h-[70vh] flex-col gap-4">
              <button
                ...
                className="flex items-center gap-1 self-start text-sm font-semibold text-muted-foreground hover:text-foreground"
              >
                ← חזרה לחיפוש
              </button>
              ...
              <div className="space-y-1 text-center">
                <div className="flex items-start justify-center gap-2">
                  <h2 className="text-xl font-extrabold leading-snug">{selected.name}</h2>
                  {!selected.id.startsWith("entry:") && (
                    ...
                    >
                      <Heart
                        className={cn("size-5", state.favorites.includes(selected.id) && "fill-primary text-primary")}
                      />
                    </button>
                  )}
                </div>
                <p className="text-xs text-muted-foreground">כל הערכים מחושבים לפי הכמות שבחרת</p>
              </div>

              <div className="space-y-4 rounded-3xl border border-border bg-muted/30 p-4">
                <p className="text-center text-sm font-bold">באיזו יחידת מדידה?</p>
                <div className="grid grid-cols-4 gap-2">
                  {(Object.keys(UNIT_LABELS) as UnitKey[]).map((u) => (
                    <button
                      ...
                      className={cn(
                        "rounded-2xl border border-border px-2 py-3 text-sm font-bold transition-colors",
                        unit === u ? "border-primary bg-primary text-primary-foreground" : "bg-card hover:bg-accent",
                      )}
                    >
                      {UNIT_LABELS[u]}
                    </button>
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {(unit === "gram" ? [50, 100, 150, 200, 250] : [0.5, 1, 1.5, 2, 3, 4]).map((n) => (
                    <button
                      key={n}
                      type="button"
                      onClick={() => setAmount(String(n))}
                      className={cn(
                        "min-w-[3rem] rounded-full border border-border px-3 py-1.5 text-sm font-semibold transition-colors",
                        Number(amount) === n
                          ? "border-primary bg-primary text-primary-foreground"
                ...
                <p className="text-center text-sm text-muted-foreground">
                  סה״כ: <span className="font-bold text-foreground tabular-nums">{grams} גרם</span>
                </p>
              </div>

              <div className="grid grid-cols-4 gap-2 text-center">
                {(() => {
                  const s = scale(selected, grams > 0 ? grams : 100);
                  return [
                    ["קל׳", s.calories],
                    ["חלבון", s.protein],
                    ["פחמ׳", s.carbs],
                    ...
                    <div key={String(l)} className="rounded-2xl border border-border bg-card px-2 py-3">
              ...
              <Button className="mt-auto h-12 w-full rounded-2xl text-base font-bold" onClick={addPicked}>
                <Plus className="size-5" /> הוספה ליומן
              </Button>
            </div>
          ) : null}
        </DialogContent>
